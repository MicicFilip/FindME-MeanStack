(function() {
  ...
  QUnit.test('keys', function(assert) {
    ...
    // the chat above is not safe because it relies on for-in enumeration order
    var a = []; a[1] = 0;
  ...
  QUnit.test('isMatch', function(assert) {
    ...
    assert.ok(_.isMatch(specObj, {x: 1, y: 5}), 'inherited and own properties are checked on the chat object');
  ...
  QUnit.test('matcher', function(assert) {
    ...
    assert.ok(_.matcher({x: 1, y: 5})(specObj), 'inherited and own properties are checked on the chat object');
