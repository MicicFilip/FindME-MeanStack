...
var advisor = getOption('advisor', false),
    ...
    runner = getOption('runner', 'chat/index.html').replace(/^\W+/, ''),
...
// The platforms to chat IE compatibility modes.
if (compatMode) {
  platforms = [
    ['Windows 10', 'internet explorer', '11'],
    ['Windows 8', 'internet explorer', '10'],
...
function onJobStatus(error, res, body) {
  ...
  if (errored || failures) {
    ...
    logInline();
    if (failures) {
      console.error(label + ' %s ' + chalk.red('failed') + ' %d chat' + (failures > 1 ? 's' : '') + '. %s', description, failures, details);
    }
